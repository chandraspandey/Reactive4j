...
import example.flowr.promise.server.ScheduledPromiseServer;

public class ScheduledPromiseClient<REQUEST, RESPONSE> implements PromiseTypeClient<REQUEST, RESPONSE> {

	private boolean isConfigured;
	private RequestType requestType 						= RequestType.PROMISE_SCHEDULED;
	...
	public ClientIdentity buildClientIdentity() throws ConfigurationException{
		...
		ClientIdentity clientIdentity = new ClientIdentity("ScheduledPromiseClient",ClientType.PERSISTENT);
	...
	public PromiseTypeServer<REQUEST, RESPONSE> buildPromiseTypeServer() throws ConfigurationException{
		...
		PromiseTypeServer<REQUEST, RESPONSE> integrationServer = 
			(PromiseTypeServer<REQUEST, RESPONSE>) new ScheduledPromiseServer();
	...
	public void run(){
		...
		try{
			...
			if(isConfigured){
				...
				System.out.println("ScheduledPromiseClient : Response = "+response);	
